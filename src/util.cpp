...
const char * const BITCOIN_CONF_FILENAME = "scriv.conf";
const char * const BITCOIN_PID_FILENAME = "scrivd.pid";
...
bool fLogTimescrivs = DEFAULT_LOGTIMESCRIVS;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "scriv" is a composite category enabling all Scriv-related debug output
            if(ptrCategory->count(string("scriv"))) {
 ...
 * suppress printing of the timescriv when multiple calls are made that don't
 * end in a newline. Initialize it to true, and hold/manage it, in the calling context.
 */
static std::string LogTimescrivStr(const std::string &str, bool *fStartedNewLine)
{
    string strScrived;
    ...
    if (*fStartedNewLine) {
        ...
        strScrived = DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nTimeMicros/1000000);
        if (fLogTimeMicros)
            strScrived += strprintf(".%06d", nTimeMicros%1000000);
        strScrived += ' ' + str;
    } else
        strScrived = str;
...
int LogPrintStr(const std::string &str)
{
    ...
    std::string strTimescrived = LogTimescrivStr(strThreadLogged, &fStartedNewLine);
    ...
    {
        // print to console
        ret = fwrite(strTimescrived.data(), 1, strTimescrived.size(), stdout);
        fflush(stdout);
    }
    else if (fPrintToDebugLog)
    {
        ...
        if (fileout == NULL) {
            assert(vMsgsBeforeOpenLog);
            ret = strTimescrived.length();
            vMsgsBeforeOpenLog->push_back(strTimescrived);
        }
        else
        {
            ...
            ret = FileWriteStr(strTimescrived, fileout);
        }
    }
    return ret;
}
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ScrivCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ScrivCore
    // Mac: ~/Library/Application Support/ScrivCore
    // Unix: ~/.scrivcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ScrivCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ScrivCore";
#else
    // Unix
    return pathRet / ".scrivcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty scriv.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override scriv.conf
