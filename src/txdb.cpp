...
static const char DB_TIMESCRIVINDEX = 's';
...
bool CBlockTreeDB::WriteTimescrivIndex(const CTimescrivIndexKey &timescrivIndex) {
    CDBBatch batch(*this);
    batch.Write(make_pair(DB_TIMESCRIVINDEX, timescrivIndex), 0);
    return WriteBatch(batch);
}

bool CBlockTreeDB::ReadTimescrivIndex(const unsigned int &high, const unsigned int &low, std::vector<uint256> &hashes) {

    boost::scoped_ptr<CDBIterator> pcursor(NewIterator());

    pcursor->Seek(make_pair(DB_TIMESCRIVINDEX, CTimescrivIndexIteratorKey(low)));

    while (pcursor->Valid()) {
        boost::this_thread::interruption_point();
        std::pair<char, CTimescrivIndexKey> key;
        if (pcursor->GetKey(key) && key.first == DB_TIMESCRIVINDEX && key.second.timescriv <= high) {
