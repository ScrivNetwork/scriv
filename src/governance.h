...
private:
    std::vector<int64_t> vecTimescrivs;
...
public:
    ...
    int64_t GetMinTimescriv()
    {
        ...
        do {
            if(vecTimescrivs[nIndex] < nMin) {
                nMin = vecTimescrivs[nIndex];
            }
    ...
    int64_t GetMaxTimescriv()
    {
        ...
        do {
            if(vecTimescrivs[nIndex] > nMax) {
                nMax = vecTimescrivs[nIndex];
            }
            nIndex = (nIndex + 1) % RATE_BUFFER_SIZE;
        } while(nIndex != nDataEnd);
        return nMax;
    ...
    double GetRate()
    {
        ...
        int64_t nMin = GetMinTimescriv();
        int64_t nMax = GetMaxTimescriv();
        if(nMin == nMax) {
            // multiple objects with the same timescriv => infinite rate
            return 1.0e10;
        }
