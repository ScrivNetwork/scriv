...
void CGovernanceManager::MasternodeRateUpdate(const CGovernanceObject& govobj)
{
    ...
    int64_t nTimescriv = govobj.GetCreationTime();
    if (GOVERNANCE_OBJECT_TRIGGER == nObjectType)
        it->second.triggerBuffer.AddTimescriv(nTimescriv);
    else if (GOVERNANCE_OBJECT_WATCHDOG == nObjectType)
        it->second.watchdogBuffer.AddTimescriv(nTimescriv);

    if (nTimescriv > GetTime() + MAX_TIME_FUTURE_DEVIATION - RELIABLE_PROPAGATION_TIME) {
...
bool CGovernanceManager::MasternodeRateCheck(const CGovernanceObject& govobj, bool fUpdateFailStatus, bool fForce, bool& fRateCheckBypassed)
{
    ...
    if(nTimescriv < nNow - 2 * nSuperblockCycleSeconds) {
        LogPrintf("CGovernanceManager::MasternodeRateCheck -- object %s rejected due to too old timescriv, masternode vin = %s, timescriv = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimescriv, nNow);
        return false;
    }

    if(nTimescriv > nNow + MAX_TIME_FUTURE_DEVIATION) {
        LogPrintf("CGovernanceManager::MasternodeRateCheck -- object %s rejected due to too new (future) timescriv, masternode vin = %s, timescriv = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimescriv, nNow);
        return false;
    }
    ...
    buffer.AddTimescriv(nTimescriv);
    ...
    if(!fRateOK)
    {
        LogPrintf("CGovernanceManager::MasternodeRateCheck -- Rate too high: object hash = %s, masternode vin = %s, object timescriv = %d, rate = %f, max rate = %f\n",
                  strHash, vin.prevout.ToStringShort(), nTimescriv, dRate, dMaxRate);
...
void CGovernanceManager::CheckPostponedObjects(CConnman& connman)
{
    ...
    for(hash_s_it it = setAdditionalRelayObjects.begin(); it != setAdditionalRelayObjects.end();) {
        ...
        if(itObject != mapObjects.end()) {
            ...
            bool fValid = (nTimescriv <= nNow + MAX_TIME_FUTURE_DEVIATION) && (nTimescriv >= nNow - 2 * nSuperblockCycleSeconds);
            bool fReady = (nTimescriv <= nNow + MAX_TIME_FUTURE_DEVIATION - RELIABLE_PROPAGATION_TIME);
