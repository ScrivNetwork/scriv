...
// Copyright (c) 2014-2017 The Scriv Core developers
...
#if defined(NDEBUG)
# error "Scriv Core cannot be compiled without assertions."
#endif
...
static std::pair<int, int64_t> CalculateSequenceLocks(const CTransaction &tx, int flags, std::vector<int>* prevHeights, const CBlockIndex& block)
{
    ...
    for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
        ...
        if (txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG) {
            ...
            // smallest allowed timescriv of the block containing the
...
bool GetTimescrivIndex(const unsigned int &high, const unsigned int &low, std::vector<uint256> &hashes)
{
    if (!fTimescrivIndex)
        return error("Timescriv index not enabled");

    if (!pblocktree->ReadTimescrivIndex(high, low, hashes))
        return error("Unable to get hashes for timescrivs");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if(pindex->pprev != NULL && pindex->pprev->nHeight > chainparams.GetConsensus().nLastBlockHeight)    
    ...
    // This rule was originally applied to all blocks with a timescriv after March 15, 2012, 0:00 UTC.
    ...
    // SCRIV : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(SCRIV): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(SCRIV): couldn't find masternode or superblock payments"),
    ...
    if (fTimescrivIndex)
        if (!pblocktree->WriteTimescrivIndex(CTimescrivIndexKey(pindex->nTime, pindex->GetBlockHash())))
            return AbortNode(state, "Failed to write timescriv index");
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (block.GetBlockTime() > GetAdjustedTime() + 2 * 60 * 60)
        return state.Invalid(error("CheckBlockHeader(): block timescriv too far in the future"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SCRIV : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(SCRIV): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(SCRIV): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Check timescriv against prev
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast())
        return state.Invalid(error("%s: block's timescriv is too early", __func__),
...
bool static LoadBlockIndexDB()
{
    ...
    // Check whether we have a timescriv index
    pblocktree->ReadFlag("timescrivindex", fTimescrivIndex);
    LogPrintf("%s: timescriv index %s\n", __func__, fTimescrivIndex ? "enabled" : "disabled");
...
bool InitBlockIndex(const CChainParams& chainparams) 
{
    ...
    // Use the provided setting for -timescrivindex in the new database
    fTimescrivIndex = GetBoolArg("-timescrivindex", DEFAULT_TIMESCRIVINDEX);
    pblocktree->WriteFlag("timescrivindex", fTimescrivIndex);
