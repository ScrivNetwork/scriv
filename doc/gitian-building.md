...
*Setup instructions for a Gitian build of Scriv Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Scriv
...
- [Building Scriv Core](#building-scriv-core)
...
Clone the git repositories for Scriv Core and Gitian.
...
in the Scriv Core repository under 'Fetch and build inputs' to install sources which require
...
To build Scriv Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Scriv Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/scriv/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ScrivNetwork/scriv
...
For example:
```bash
URL=https://github.com/crowning-/scriv.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit scriv=${COMMIT} --url scriv=${URL} ../scriv/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit scriv=${COMMIT} --url scriv=${URL} ../scriv/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit scriv=${COMMIT} --url scriv=${URL} ../scriv/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the scriv git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../scriv/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/ScrivNetwork/scriv/scriv-detached-sigs.git

BTCPATH=/some/root/path/scriv.git
SIGPATH=/some/root/path/scriv-detached-sigs.git

./bin/gbuild --url scriv=${BTCPATH},signature=${SIGPATH} ../scriv/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/scriv-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/scriv-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/scriv-osx-build.assert
```
...
[scriv/gitian.sigs]( https://github.com/ScrivNetwork/scriv/gitian.sigs/) repository, or if that's not possible to create a pull
request.
